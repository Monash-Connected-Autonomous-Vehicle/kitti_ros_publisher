...
def load_oxts_packets_and_poses(oxts_files:list, origin:np.array = None, scale:float = None) -> Tuple[np.array, list, list, np.array]:
    """Generator to read OXTS ground truth data.
       Poses are given in an East-North-Up coordinate system 
       whose origin is the first GPS position.

    Args:
        oxts_files (list): _description_
        origin (np.array, optional): Origin of the global coordinate system (first GPS position).
        scale (float, optional): Scale for Mercator projection (from first lat value).

    Returns:
        Tuple[np.array, list, list, np.array]: oxts transformation matrix, oxtf angular velocity, oxtf linear acceleration, origin.
    """
    ...
    for filename in oxts_files:
        with open(filename, 'r') as f:
            for line in f.readlines():
                ...
                oxtf_vel = [packet.wx, packet.wy, packet.wz]

    return oxts_tf, oxtf_vel, oxtf_acc, origin
